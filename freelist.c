...
StrategyGetBuffer(void)
{
	...
	while (StrategyControl->firstFreeBuffer >= 0)
	{
		...
		//chengbin: add a line to set Prev pointer.
		 ...
		 */
		//LockBufHdr(buf);
		//Chengbin: then we select a buf from free list in no one use that buf
		//for(;;)
		//{
		LockBufHdr(buf);
			if (buf->refcount == 0)// && buf->usage_count == 0) 
				{elog(LOG, "Get buf %d\n", buf->buf_id);//chengbin: add elog
				return buf;}
			/*else 
			{
				...
				if (buf->refcount == 0){elog(LOG, "Get buf %d\n", buf->buf_id);//chengbin: add elog
				return buf;}
			}*/
		UnlockBufHdr(buf);
		//}
...
StrategyFreeBuffer(volatile BufferDesc *buf, bool at_head)
{
	...
	{
		if (at_head)
		{//AT HEAD, the buf prev should point to END
			buf->freePrev = FREE_END_OF_LIST;
			buf->freeNext = StrategyControl->firstFreeBuffer;
			if (buf->freeNext < 0)
				StrategyControl->lastFreeBuffer = buf->buf_id;
			else
				BufferDescriptors[StrategyControl->firstFreeBuffer].freePrev = buf->buf_id;
			StrategyControl->firstFreeBuffer = buf->buf_id;
			//chengbin: This time we add a buffer.
			 elog(LOG, "Add buf %d\n", buf->buf_id);
		}
		else
		{
			//at the end prev should point to last free buffer
