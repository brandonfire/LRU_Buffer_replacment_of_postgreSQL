...
static void Movebuffer(volatile BufferDesc *buf);
...
/* Chengbin: The function of Movebuffer.
* It takes a buf value and move it out from the double linked list;
*
*
*/
void Movebuffer(volatile BufferDesc *buf)
{
	 LWLockAcquire(BufFreelistLock, LW_EXCLUSIVE);
	 if(buf->freeNext > 0 && buf->freePrev > 0)
	 {
		BufferDescriptors[buf->freeNext].freePrev = buf->freePrev;//delete the buf from the double linked list
        	BufferDescriptors[buf->freePrev].freeNext = buf->freeNext;
        	buf->freeNext = FREENEXT_NOT_IN_LIST;//point to the empty pointer
        	buf->freePrev = FREEP_NOT_IN_LIST;
	 }
...
PinBuffer(volatile BufferDesc *buf)
{
	...
	{
		LockBufHdr(buf);
		/* chengbin:  For LRU pinbuffer,we do not always keep the buf in freelist. add a check point for whether refcount==0 if is free. move out it*/
		//if(buf->refcount == 0)
		//{
		//Movebuffer(buf);
...
UnpinBuffer(volatile BufferDesc *buf, bool fixOwner, bool normalAccess)
{
	...
	if (PrivateRefCount[b] == 0)
	{
		...
		//chengbin: once there is no one use the buffer, put it back to free list tail
		...
		//}
		//end
			//chengbin: once there is no one use the buffer, drag it out from the linked list and put it back to free list tail
		else if(buf->refcount ==0)
		{
			UnlockBufHdr(buf);
			LWLockAcquire(BufFreelistLock, LW_EXCLUSIVE);
	 			if(buf->freeNext >=0 && buf->freePrev >=0)//if this buf is in midlle. drag to tail
	 		{
				BufferDescriptors[buf->freeNext].freePrev = buf->freePrev;//delete the buf from the double linked list
        		BufferDescriptors[buf->freePrev].freeNext = buf->freeNext;
        		buf->freeNext = FREENEXT_NOT_IN_LIST;//point to the empty pointer
				...
				StrategyFreeBuffer(buf,false);
				//UnlockBufHdr(buf);
	 		} else if(buf->freeNext >=0 && buf->freePrev == FREE_END_OF_LIST)//if this buf is at the head. drag to tail
	 		{
	 			BufferDescriptors[buf->freeNext].freePrev = FREE_END_OF_LIST;//delete the buf from the double linked list
        		//BufferDescriptors[buf->freePrev].freeNext = buf->freeNext;
				...
				StrategyFreeBuffer(buf,false);
				//UnlockBufHdr(buf);
	 		} else// if this buf is at the tail. do nothing
	 		{
	 			LWLockRelease(BufFreelistLock);
	 			//UnlockBufHdr(buf);
	 		}
