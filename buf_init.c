...
InitBufferPool(void)
{
	...
	else
	{
		...
		for (i = 0; i < NBuffers; buf++, i++)
		{
			 ...
			 */
			buf->freeNext = i + 1;
			//chengbin: initial freePrev pointer point to the i-1 buf.
			buf->freePrev = i - 1;
			//END
		...
		//Chengbin: we have two ends now. both need to define the end.
		BufferDescriptors[0].freePrev = FREE_END_OF_LIST;
